...
So i had some problem with this part. I went online for some hints where I saw the idea to split the graph into subgraphs. Then when trying to calculate the number of paths I noticed that it follows a pattern. Tribonnaci. I.e the next number is the sum of the three previous numbers. I combined these two ideas by first finding all the subgraphs. A new graph starts when there is a space of three between the current andprevious jolt ( since then there is only one path between them). I put the length of all the subgraphs in an array. Then I calculate tribonnaci up to the largest number in the array and put those numbers in an array. We can then loop through the array with the subgraphs and use their length as a pointer to the tribonnaci graph. Multiply them togheter and get the answer.

We can probably also calculate tribonnaci up to the total numbers of paths and get the answer but I thin this is quicker.

# Answers

| Part 1 | Part 2            |
| ------ | ----------------- |
| `3000` | `193434623148032` |
