...
an easy way to do this is to use the formula `busId - (arrivalTime % bus)` This gives the time you have to wait for the bus where arivaltime is the time you arrive.
...
You cant really brute force your way through this one So we have to be clever about this.
We start by finding a time that fits for the first two buses by incrementing the time by the first bus id. After we found a time that matches we increment by the least common multiple. Since all the bus ids are prime numbers we can just multiply them togheter to get the next increment.
