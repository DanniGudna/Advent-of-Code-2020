...
Passports cannot contain any field that is not expected. So we can create objects out of the passport strings and count the number of keys. If the keys are 7 then it is a valid passport. When we create the passport object then we do not add a key or value for `cid`. That way the count is accurate
...
We already make the passport lines into object so applying the rules to each field is eazy. We can also check first if it is a real passpot to lower the number of iterations.
